//
//  OccupationTypeViewController.m
//  BVSEnquiry
//
...
@interface OccupationTypeViewController ()

@end

@implementation OccupationTypeViewController
@synthesize delegate,pickerView,selectedString, pickerArray;

+ (OccupationTypeViewController*)sharedSingleton
{
    static OccupationTypeViewController * sharedSingleton;
    if(!sharedSingleton)
    {
        @synchronized(sharedSingleton)
        {
            sharedSingleton = [[OccupationTypeViewController alloc]initWithNibName:@"OccupationTypeViewController" bundle:nil];
        }
    }
...
-(void)createDoneButton
{
    ...
    doneButton.frame=CGRectMake(self.view.frame.size.width/2-50,pickerView.frame.origin.y + pickerView.frame.size.height + 10, 100, 40);//pickerView.frame.origin.y + pickerView.frame.size.height+5
...
- (IBAction)doneButtonClicked:(id)sender
{
    ...
    [delegate sendSelectedApplyOccupationTypePickerValue:selectedString];
